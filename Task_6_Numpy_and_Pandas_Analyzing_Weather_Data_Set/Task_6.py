...
def categorical(df):
    ...
    categorical_var = df.select_dtypes(exclude=np.number).columns.tolist()
...
def numerical(df):
    ...
    numerical_var = df.select_dtypes(include=np.number).columns.tolist()
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    aggregated_value = pd.pivot_table(df,columns=date_col,values=agg_col,aggfunc=agg)
